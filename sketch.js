...
if(keyWentDown("space")){
  bullet = createSprite(displayWidth-1150,jotaro.y-30,20,10)
  bullet.velocityX = 20
  bulletGroup.add(bullet)
 jotaro.depth=bullet.depth
 jotaro.depth=jotaro.depth+2
...
if(zombieGroup.isTouching(jotaro)){

 for(var i=0;i<zombieGroup.length;i++){     
      
  if(zombieGroup[i].isTouching(jotaro)){
       zombieGroup[i].destroy()
       } 
...
//destroy zombie and player and display a message in gameState "won"
  ...
  textSize(100)
  fill("yellow")
  text("You clapped zombies ",400,400)
  zombieGroup.destroyEach();
  jotaro.destroy();
